...
int main(int argc, char* argv[]) {
    ...
    // Map to hold variants, automatically sorted by chromosome and position.
    ...
    #pragma omp parallel
    {
        ...
        for (size_t i = 0; i < lines.size(); ++i) {
            ...
            for (const auto& [position, ref_base, alt_base] : mismatches) {
                ...
                var.read_names.insert(read_name);
            }
        }
        ...
        #pragma omp critical
        {
            ...
            for (const auto& kv : local_variants_map) {
                ...
                if (var.read_names.empty()) {
                    ...
                    var.read_names.insert(kv.second.read_names.begin(), kv.second.read_names.end());
                }
            }
        }
    }
    ...
    for (const auto& [_, var] : variants_map) {
        out << var.ref_name << '\t'
            << var.position << '\t'
            << '.' << '\t' // ID is '.' if unknown
            << var.ref_base << '\t'
            << var.alt_base << "\t.\t.\t.\tGT\t";
        for (const auto& read : reads) {
            out << (var.read_names.find(read) != var.read_names.end() ? "1" : "0") << '\t';
        }
        out << '\n';
    }
