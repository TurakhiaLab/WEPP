...
### <a name="dashboard"><a> Running the WEPP Dashboard
The WEPP dashboard provides an interactive interface for exploring inferred haplotypes, lineage abundances, and unaccounted alleles. You can either run it during a WEPP analysis (useful on a server) or launch it locally using an existing `results/` directory.

**Option 1: Run the dashboard during a WEPP run (server / remote)** \
Enable the dashboard in your WEPP command:
```
run-wepp --config ... DASHBOARD_ENABLED=True --cores <N> --use-conda
```
If you are running WEPP on a remote machine, use SSH port forwarding to access the dashboard in your local browser:
```
ssh -L 8080:localhost:80 user@remote_host
```
Open the dashboard in your browser at:
```http://localhost:8080 ```
> Replace 8080 with any available local port if needed.
<br/>

**Option 2: Visualize results locally using Docker**\
If WEPP was executed on a server/cluster, you can copy the outputs to your local machine and launch the dashboard locally.

Step 1: Copy results to your local machine

Run the following command on your local computer, replacing the placeholders with your server details:
```
scp -r user@remote_host:/path/to/WEPP/results /path/to/local/results
```
> Replace /path/to/local/results with any directory on your local machine where you want to store the WEPP outputs.


Step 2: Pull the WEPP dashboard Docker image
```
...
```
  docker run -it \
  -v "/path/to/local/results:/app/taxonium_backend/results" \
  -p 8080:80 \
...
http://localhost:8080
```
> The dashboard runs inside the container on port 80.
> You may replace 8080 with any available port on your machine.
