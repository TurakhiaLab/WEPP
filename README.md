...
- [Quick Start](#example)
     ...
     - [Example-2: SARS-CoV-2 Dataset (Takes Longer to Run)](#sars_cov_2_example)
- [User Guide](#guide)
...
### <a name="sars_cov_2_example"></a> Example - 2: SARS-CoV-2 Dataset (Takes Longer to Run)
**Step 1:** Download the test dataset
```bash
...
All results generated by WEPP can be found in the `results/SARS_COV_2_real` directory.
...
└───📁data                                   # [User Created] Contains data to analyze 
    ├───📁SARS-CoV-2_test_1                  # SARS-CoV-2 run wastewater samples - 1
         ├───sars_cov_2_reads.fastq.gz       # Single-ended reads 
         ├───sars_cov_2_reference.fa
         ├───mask.bed                        # OPTIONAL 
         └───sars_cov_2_mat.pb.gz
    ├───📁SARS-CoV-2_test_2                  # SARS-CoV-2 run wastewater samples - 2
         ├───sars_cov_2_reads_R1.fastq.gz    # Paired-ended reads
         ├───sars_cov_2_reads_R2.fastq.gz    # Paired-ended reads
         ├───sars_cov_2_reference.fa 
...
```bash
snakemake --config DIR=SARS-CoV-2_test_1 FILE_PREFIX=test_run_1 --cores 32 --use-conda
```

2. Overriding MIN_Q, DASHBOARD_ENABLED, and TAXONIUM_FILE through command line.
```bash
snakemake --config DIR=SARS-CoV-2_test_2 FILE_PREFIX=test_run_2 MIN_Q=25 DASHBOARD_ENABLED=True TAXONIUM_FILE=sars_cov_2_taxonium.jsonl.gz --cores 32 --use-conda
```
...
```bash
snakemake --config DIR=SARS-CoV-2_test_1 FILE_PREFIX=test_run_1 DASHBOARD_ENABLED=True --cores 32 --use-conda --forcerun dashboard_serve
```
