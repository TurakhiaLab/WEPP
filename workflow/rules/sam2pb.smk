...
rule sam2pb:
    ...
    shell:
        ...
        "./build/wepp sam2PB -T {threads} -i " + config["TREE"] + " -p '{wildcards.FILE_PREFIX}' -d '{wildcards.DIR}'" + " -m " + str(config.get("MAX_READS", str(int(1e9)))) + " -a " + str(config["MIN_AF"]) + " -q " + str(config["MIN_Q"]) 